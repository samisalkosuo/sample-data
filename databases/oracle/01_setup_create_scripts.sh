...
create table ${ORACLE_USER}.phonenumbers (id char(17) unique not null, phonenumber char(10));
EOF

cat << EOF > /opt/oracle/scripts/startup/04-insert-data.sh
#!/bin/sh
sqlldr userid=${ORACLE_USER}/${ORACLE_PWD} data=/tmp/phonenumbers.csv control=/opt/oracle/scripts/startup/load-phonenumbers.ctl
EOF
